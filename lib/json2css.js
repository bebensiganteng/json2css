...
Input.prototype = {
  ...
  'ensureOffsetAndPx': function () {
    ...
    this.input.forEach(function (item) {
      ...
      item.o_x_half = -(item.x * .5);
      item.o_y_half = -(item.y * .5);
      item.t_h_half = item.total_height * 0.5;
      item.t_w_half = item.total_width * 0.5;
      item.h_half   = item.height * 0.5;
      item.w_half   = item.width * 0.5;
      ...
      ['x', 'y', 'offset_x', 'offset_y', 'height', 'width', 'total_height', 'total_width', 'o_x_half', 'o_y_half', 't_h_half', 't_w_half', 'h_half', 'w_half'].forEach(function (key) {
        px[key] = item[key] + 'px';
      });
    });
  },
...
addTemplate('sass', require(templatesDir + '/sass.template.js'));
